...
int main() {
    ...
    execute_command("sudo useradd -m john"); // -m creates a home directory

    // Set a password for the user "john"
    printf("Setting a password for 'john'...\n");
    execute_command("echo 'john:password' | sudo chpasswd");

    // Grant permission to run /usr/bin/touch as root
    ...
    execute_command("sudo chmod 440 /etc/sudoers.d/john");
